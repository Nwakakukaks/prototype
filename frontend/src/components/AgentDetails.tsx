...
const AgentDetails = ({ ens, chain, resources = [] }: AgentDetailsProps) => {
    return (
        <div className="bg-card rounded-lg p-4 h-full overflow-hidden">
            <h2 className={`${pixelify_sans.className} text-xl mb-4 text-orange-900`}>Agent Details</h2>

            <div className="space-y-4 text-black">
                <div>
                    <h3 className="text-sm font-semibold text-orange-900">ENS</h3>
                    <p className="text-sm">{ens || 'Not set'}</p>
                </div>

                <div>
                    <h3 className="text-sm font-semibold text-orange-900">Resources</h3>
                    {resources.length > 0 ? (
