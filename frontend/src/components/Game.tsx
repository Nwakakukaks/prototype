...
const Game = ({
  ...
  const drawGame = useCallback(() => {
    ...
    characters.forEach((character, index) => {
      ...
      if (
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(characterX, characterY, characterWidth, characterHeight);
      }
    });
  ...
  useEffect(() => {
    if (!isInitialized) {
      console.debug(
        "Animation loop not started because game is not initialized"
      );
      return;
    }
          ...
          if (Date.now() >= playerState.ai.actionEndTime) {
            ...
            if (playerState.ai.action === "paused") {
              ...
              playerState.ai.actionEndTime =
                Date.now() +
                getRandomInt(AI_MOVE_DURATION_MIN, AI_MOVE_DURATION_MAX);
              playerState.direction =
  ...
  const controlledPlayerState =
    controlledCharacterIndex !== null
      ? playerStates[controlledCharacterIndex]
      : null;
