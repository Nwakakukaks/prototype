...
export class Character {
  ...
  private initializeWebSocket(sessionId: string) {
    ...
    try {
      ...
      console.error(
        `WebSocket creation error for character ${this.name}:`,
        error
      );
    ...
    this.websocket.onclose = (event) => {
      ...
      console.log(
        `Attempting to reconnect WebSocket for character ${this.name}`
      );
      setTimeout(() => {
  ...
  drawSpeechBubble(
    ...
    const targetWidth = Math.min(
      maxWidth,
      Math.max(minWidth, totalWidth + padding * 2)
    );
  ...
  private isOverlapping(
    bubble1: CharacterBounds,
    bubble2: CharacterBounds
  ): boolean {
    return !(
