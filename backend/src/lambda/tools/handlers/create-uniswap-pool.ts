...
const ELECTRO_RPC_URL = process.env.ELECTRO_RPC_URL as string;
...
export async function createUniswapPool({
    ...
    try {
        ...
        // Connect to Provider
        logConsole.info("Connecting to RPC provider with URL: ", ELECTRO_RPC_URL);
        const provider = new ethers.JsonRpcProvider(ELECTRO_RPC_URL);
