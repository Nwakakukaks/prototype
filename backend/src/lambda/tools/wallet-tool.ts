...
export const walletToolDescription = [
        ...
        json: {
          ...
          properties: {
            createdBy: {
              type: "string",
              description:
                "The user that is creating the wallet. (starts with 'user_' and will look something like 'user_2nCKty8ggdPrOyvsNgupp1oDd9Y')",
            },
            characterId: {
              type: "string",
        ...
        json: {
          ...
          properties: {
            createdBy: {
              type: "string",
              description:
                "The user that is getting the wallet. (starts with 'user_' and will look something like 'user_2nCKty8ggdPrOyvsNgupp1oDd9Y')",
            },
            characterId: {
              type: "string",
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Deploys a ERC20 smart contract to the Base network and returns the contract address and the deployer's balance of the token",
      inputSchema: {
        json: {
          type: "object",
          properties: {
            ...
            createdBy: {
              type: "string",
              description:
                "The user that is deploying the contract. (starts with 'user_' and will look something like 'user_2nCKty8ggdPrOyvsNgupp1oDd9Y'))",
            },
            characterId: {
              type: "string",
              description:
                "The character that is deploying the contract (This will always be Qwen) as he is the one deploying the contract on behalf of the asker.",
            },
            network: {
              type: "string",
              description:
                "The network to deploy the contract on. This will default to 'base' if not provided.",
            },
            tokenSymbol: {
              type: "string",
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Gets the S balance of the wallet along with a friendly concise message",
      inputSchema: {
        json: {
          type: "object",
          properties: {
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Requests funds from the user that will be sent to the agents wallet",
      inputSchema: {
        json: {
          type: "object",
          properties: {
            ...
            sendersWalletAddress: {
              type: "string",
              description:
                "The wallet address of the user that is sending the funds. This is not the wallet address of the character.",
            },
          },
          required: [
            "createdBy",
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Creates and deploys an NFT on Zora given a name and returns the Zurl URL for Monad to promote",
      inputSchema: {
        json: {
          type: "object",
          properties: {
            ...
            imageKey: {
              type: "string",
              description:
                "The key of the image to use for the NFT metadata. This is the key of the image that was created with the Create_Image_Tool.",
            },
            description: {
              type: "string",
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Checks if a wallet already has a Basename. If not, it registers one and sets the display name and avatar.",
      inputSchema: {
        json: {
          type: "object",
          properties: {
            ...
            baseNamePrefix: {
              type: "string",
              description:
                "The prefix for the Basename, e.g., 'baseNamePrefix-<characterId>'.",
            },
            imageUrl: {
              type: "string",
  ...
  },
  {
    toolSpec: {
      ...
      description:
        "Gets the ERC20 token balance of the wallet given a token address",
      inputSchema: {
        json: {
          type: "object",
          properties: {
        ...
        json: {
          ...
          properties: {
            createdBy: {
              type: "string",
              description:
                "The user that is transferring the funds. (starts with 'user_' and will look something like 'user_2nCKty8ggdPrOyvsNgupp1oDd9Y')",
            },
            characterId: {
              type: "string",
              description:
                "The character that is transferring the funds, we will use this to look up the wallet address in the tool.",
            },
            destinationWalletAddress: {
              type: "string",
              description:
                "The destination character's wallet address to transfer the funds to.",
            },
            amountInWei: {
              type: "string",
...
export async function walletToolHandler(
  ...
  logConsole.info(
    `Wallet Tool fired with response: ${JSON.stringify(response)} ${JSON.stringify(conversation)}`
  );
  ...
  const toolUseContent = response.content.find(
    (content) => "toolUse" in content && content.toolUse
  );

  // If we have text content and toolUse with required fields, send the character message
  if (
    textContent?.text &&
    toolUseContent?.toolUse?.input?.characterId &&
    toolUseContent?.toolUse?.input?.sessionId
  ) {
    try {
      const { characterId, sessionId } = toolUseContent.toolUse.input;
      logConsole.info(
        `Sending message to character: ${characterId} with sessionId: ${sessionId}`
      );
